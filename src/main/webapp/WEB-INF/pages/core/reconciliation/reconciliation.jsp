...
<c:url var="noIndexUsedAsKeyUrl" value="https://reactjs.org/redirect-to-codepen/reconciliation/no-index-used-as-key"/>
...
<lt:layout cssClass="page hello-world-example-page">
  ...
  <br/>
  <h3>3.8.2.3 Элементы компонентов одинакового типа</h3>

  <wg:p>
    Когда компонент обновляется, экземпляр остается неизменным, так что во время
    отрисовок состояние сохраняется. React обновляет свойства экземпляра компонента,
    чтобы соответствовать новому элементу (виртуальному узлу дерева) и вызывает
    методы <code>componentWillReceiveProps()</code> и <code>componentWillUpdate()</code> в этом экземпляре.
  </wg:p>
  ...
  <br/>
  <h3>
    <a class="anchor" name="recursing-on-children">
  ...
  <wg:p>
    Реорганизация также может вызывать проблемы с состоянием компонента, когда в
    качестве ключей используются индексы. Экземпляры компонентов обновляются и повторно
    используются на основе их ключа. Если ключ является индексом, перемещение элемента
    изменяет его. В результате состояние компонента, отрисовывающего, например, управляемые <code>input</code>,
    может быть смешено и/или обновлено неожиданными путями.
  </wg:p>

  <wg:p>
    <wg:link href="${indexUsedAsKeyUrl}">Здесь</wg:link> в CodePen находится пример с проблемами, которые могут быть вызваны
    использованием индексов в качестве ключей, а <wg:link href="${noIndexUsedAsKeyUrl}">здесь</wg:link> находится обновленная версия
    того же примера, показывающая, как избежать использования индексов, поскольку ключи
    будут исправлять все эти проблемы: переупорядочения, сортировки и добавления.
  </wg:p>

  <br/>
  <h3>3.8.2.6 Компромиссы</h3>
