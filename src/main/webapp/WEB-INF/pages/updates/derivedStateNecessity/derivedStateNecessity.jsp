...
<lt:layout cssClass="page derived-state-necessity-page">
    ...
    <wg:p>
        ...
        двух аккаунтов с тем же адресом электронной почты input не сможет быть сброшен.
    ...
    <a name="#fully-controlled-component"></a>
    <br/>
    ...
    <a name="#fully-uncontrolled-component"></a>
    <br/>
    <h3>Рекомендация: полностью неконтролируемый компонент с ключом key</h3>
    
    <wg:p>
        В качестве другой альтернативы может выступать решение, где наш компонент
    ...
    <app:alert title="Внимание!" type="warning">
        Хотя такое решение может показаться медленным, разница в производительности обычно незначительная.
    ...
    <wg:p>
        <ul>
            <li><b>Рекомендация: сбрасывать <i>все внутреннее состояние</i> используя атрибут</b> <code>key</code>.</li>
    ...
    <wg:p>
        ...
        мы можем упростить работу с помощью <code>PureComponent</code> и перемещения операции фильтрации в
