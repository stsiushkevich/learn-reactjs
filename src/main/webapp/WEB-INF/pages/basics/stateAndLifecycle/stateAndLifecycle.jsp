...
<lt:layout cssClass="page hello-world-example-page">
    ...
    <wg:p>1. Заменим <code>this.props.value</code> на <code>this.state.value</code> в методе <code>render()</code>:</wg:p>
    ...
    <wg:p>Нам необходимо устанавливать таймер каждый раз, когда <code>Timer</code>
        отрисовывается в DOM в первый раз. В React это называется «<b>монтированием/монтажом</b>».</wg:p>
    ...
    <wg:p>Мы будем очищать таймер в методе <code>componentWillUnmount()</code> жизненного цикла:</wg:p>

    <ce:code-example-12/>

    <wg:p>В итоге, мы реализуем метод <code>increment()</code>, который выполняется каждую секунду.</wg:p>
    ...
    <wg:p>
        <ol>
            ...
            <li>Каждую секунду браузер вызывает метод <code>increment()</code>. Внутри него <code>Timer</code>
    ...
    <wg:p>Компонент <code>ClockFace</code> хотел бы получать <code>value</code> в
        своих свойствах и не хотел бы знать, пришло ли оно из состояния компонента <code>Timer</code>,
        из свойств компонента <code>Timer</code> или было указано вручную:</wg:p>
    ...
    <wg:p>В приложениях React, независимо от того, обладает ли компонент состоянием или нет – оно рассматривается как
        деталь реализации этого компонента, который может изменяться со временем. Вы можете использовать компоненты без
