...
<lt:layout cssClass="page hello-world-example-page">
	...
	<wg:p>Часто несколько компонентов должны отражать одни и те же изменения данных. В таких случаях
		рекомендуется передавать состояние «вверх» по иерархии их ближайшему общему предку.
		Давайте посмотрим как это работает на деле.</wg:p>

	<wg:p>В этом разделе мы создадим полицейский радар скорости, который
	...
	<wg:p>В React совместно используемое состояние достигается путем передачи его
	...
	<wg:p>Мы знаем, что свойства используются только для чтения. Когда скорость <code>speed</code> была в
	...
	<wg:p>Как правило, в React это обычно решается путем создания «контролируемого» компонента.
		...
		из своего родительского компонента <code>SpeedRadar</code>.</wg:p>
	...
	<wg:p>Состояния установщиков синхронизированы, так как их значения вычислены из одного и того же состояния:</wg:p>
