...
<lt:layout cssClass="page hello-world-example-page">
	...
	<p>Мы знаем, что свойства используются только для чтения. Когда скорость <code>speed</code> находилась в
		...
		как свойство в объекте <code>props</code>, поэтому <code>SpeedSetter</code> больше не имеет над ней контроля.</p>
	...
	<p>Как правило, в React это обычно решается путем создания «контролируемого» компонента.
		...
		так и компонент <code>SpeedSetter</code> может принимать свойства <code>speed</code> и <code>onChangeSpeed</code>
	...
	<p>Обратите внимание, что нет никакого особого смысла в названиях свойств
		<code>speed</code> и <code>onChangeSpeed</code> в пользовательских компонентах. Мы могли бы назвать их
		как угодно, например, <code>value</code> и <code>onChange</code>, что не противоречит общему соглашению.</p>

	<p>Свойства <code>onChangeSpeed</code> и <code>speed</code> будут предоставлены совместно родительским
	...
	<p>Перед погружением в анализ изменений в компоненте <code>SpeedRadar</code>, давайте прорезюмируем наши изменения в
		...
		изменение скорости, мы сейчас вызываем <code>this.props.onChangeSpeed()</code>, который будет
	...
	<p>
		<ul>
			...
			<li>Метод <code>onChange</code> в компоненте <code>SpeedSetter</code> вызывает
				<code>this.props.onChangeSpeed()</code> с новым требуемым значением.
				Эти свойства, включая <code>onChangeSpeed</code>, были предоставлены его
				родительским компонентом - <code>SpeedRadar</code>.</li>

			<li>Когда <code>SpeedRadar</code> отрисовывался в предыдущий раз, он указал, что <code>onChangeSpeed</code>
