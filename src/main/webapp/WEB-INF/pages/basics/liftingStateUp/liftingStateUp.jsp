...
<lt:layout cssClass="page hello-world-example-page">
	...
	<br/>
	<h2>2.11.1 Подъём состояния выше по иерархии</h2>

	<p>На данный момент оба компонента <code>SpeedSetter</code> независимо
		хранят свои значения в локальном состоянии:</p>

	<ce:code-example-3/>

	<p>Тем не менее нам нужно два этих установщика синхронизировать между собой. Когда
		обновится установщик «<b>км/ч</b>» - установщик «<b>миль/ч</b>» отобразит
		конвертированное значение, и наоборот.</p>

	<p>В React совместно используемое состояние достигается путем передачи его
		...
		Мы удалим локальное состояние из <code>SpeedSetter</code> и перенесем его в <code>SpeedRadar</code>.</p>

	<p>Если <code>SpeedRadar</code> владеет совместно используемым состоянием, то он становится
		...
		<code>SpeedRadar</code>, оба установщика скорости всегда будут синхронизированы.</p>

	<p>Давайте посмотрим как это работает пошагово.</p>
	...
	<p>Для начала мы заменим <code>this.state.speed</code> на <code>this.props.speed</code>
		...
		передать его из компонента <code>SpeedRadar</code> в будущем:</p>

	<ce:code-example-4/>

	<p>Мы знаем, что свойства используются только для чтения. Когда скорость <code>speed</code> была в
		локальном состоянии, компонент <code>SpeedSetter</code> мог просто вызвать <code>this.setState()</code>,
		чтобы ее изменить. Однако, так как сейчас скорость приходит из родительского компонента
		как свойство в <code>prop</code>, <code>SpeedSetter</code> теперь не имеет над ней контроля.</p>

	<p>Как правило, в React это обычно решается путем создания «контролируемого» компонента.
		Так же как DOM-элемент <code>&lt;input&gt;</code> принимает и <code>value</code>, и <code>onChange</code> свойства,
		так и компонент <code>SpeedSetter</code> может принимать свойства <code>speed</code> и <code>onSpeedChange</code>
		из своего родительского компонента <code>SpeedRadar</code>.</p>

	<p>Сейчас, когда <code>SpeedSetter</code> захочет обновить свою скорость <code>speed</code>,
		он вызовет <code>this.props.onSpeedChange</code>:</p>

	<ce:code-example-5/>

	<p>Обратите внимание, что нет никакого особого смысла в названиях свойств
		<code>speed</code> и <code>onSpeedChange</code> в пользовательских компонентах. Мы могли бы назвать их
		как угодно, например, <code>value</code> и <code>onChange</code>, что не противоречит общему соглашению.</p>

	<p>Свойства <code>onSpeedChange</code> и <code>speed</code> будут предоставлены совместно родительским
		...
		Мы посмотрим на новую реализацию <code>SpeedRadar</code> очень скоро.</p>
	
	<ad:ad-content-banner-1/>

	<p>Перед погружением в анализ изменений в компоненте <code>SpeedRadar</code>, давайте прорезюмируем наши изменения в
		...
		предоставляться компонентом <code>SpeedRadar</code>:</p>
	...
	<p>К примеру, если мы вводим <code>40</code> в установщик «Км/ч», состояние компонента <code>SpeedRadar</code> будет:</p>

	<ce:code-example-7/>

	<p>Если далее мы введем в поле «Миль/ч» значение <code>80</code>, состояние <code>SpeedRadar</code> будет:</p>

	<ce:code-example-8/>

	<p>Мы могли бы хранить значения обоих установщиков, но это оказывается необязательным.
		...
		скорости <code>speed</code> и его единице измерения <code>unit</code>.</p>

	<p>Состояния установщиков синхронизированы, так как их значения вычислены из одного и того же состояния:</p>
	...
	<p>Сейчас не имеет значения в какое поле вы вводите значение: <code>this.state.speed</code> и <code>this.state.unit</code> в
		...
		вычислено на основании первоначального.</p>

	<p>Давайте прорезюмируем, что происходит, когда мы редактируем <code>input:</code></p>

	<p>
		<ul>
			<li>React вызывает функцию, указанную в атрибуте <code>onChange</code> DOM-элемента <code>&lt;input&gt;</code>.
	...
	<p>Каждое обновление проходит через эти же самые шаги, так что элементы
		<code>&lt;input&gt;</code> всегда остаются синхронизированными.</p>

	<br/>
	<h2>2.11.2	Извлеченные уроки</h2>

	<p>Должен существовать лишь один «<b>источник истины</b>» для любых данных, которые изменяются в
		...
		компонентами, вам следует полагаться на нисходящий поток данных.</p>

	<p>Поднятие состояния влечет за собой написание более «шаблонного» кода, чем подходы,
		использующие двойную привязку, но, как вознаграждение, на поиск и изоляцию багов уходит
		меньше работы. Как только какое-либо состояние «начало жить» в каком-нибудь компоненте и
	...
	<p>Если что-либо может быть извлечено и из состояния, и из свойств, возможно, это не должно
		...
		введенных пользователем.</p>

	<p>Когда вы видите что-то неправильное в UI, вы можете использовать
		...
		ответственный за обновление состояния. Это позволит вам отследить баги вплоть до их источника.</p>
</lt:layout>
