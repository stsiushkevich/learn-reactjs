...
<div class="page thinking-in-react-page">
    ...
    <p class="codepen" data-height="600" data-theme-id="0" data-default-tab="js,result" data-user="stzidane"
       data-slug-hash="BeRQxV"
       style="height: 265px; box-sizing: border-box; display: flex; align-items: center; justify-content: center; border: 2px solid; margin: 1em 0; padding: 1em;"
       data-pen-title="Thinking In React: Step 4">
        <span>See the Pen <a href="https://codepen.io/stzidane/pen/BeRQxV/"> Thinking In React: Step 4</a> by
            Sergey (<a href="https://codepen.io/stzidane">@stzidane</a>) on <a href="https://codepen.io">CodePen</a>.
        </span>
    </p>
    <script async src="https://static.codepen.io/assets/embed/ei.js"></script>
    ...
    <p>
        Итак, мы определили минимальный набор данных состояния приложения. Теперь нам нужно
        выяснить, какой компонент должен владеть этим состоянием и изменять его.
    </p>

    <p>
        Помните: React предоставляет односторонний поток данных по иерархии компонентов.
        Иногда не сразу понятно, какой компонент каким состоянием должен владеть, особенно новичкам.
    ...
    <p>
        Итак мы решили, что наше состояние будет жить в компоненте <code>FilterableProductTable</code>.
        Во-первых, добавьте свойство экземпляра <code>this.state = {filterText: '', inStockOnly: false}</code> в
        конструктор <code>FilterableProductTable</code>, чтобы отобразить начальное состояние вашего приложения. Затем
    ...
    <p>
        Сейчас вы можете проверить, как будет вести себя приложение: установите
        <code>filterText</code> в значение <code>"ball"</code>, затем обновите приложение.
        Вы увидите, что таблица с данными обновляется правильно.
    </p>
    
    <br/>
    <br/>
    <div class="gray-line"></div>
    <h2>2.13.6 Шаг 5: Добавим обратный поток данных</h2>
    <br/>
    ...
    <p>
        ...
        компонентам формы в глубине иерархии нужно обновлять состояние в <code>FilterableProductTable</code>.
    </p>

    <p>
        React делает этот поток данных явным, чтобы было легче понять, как работает ваша программа.
        Однако для этого требуется немного больше кода, чем в традиционной двусторонней привязке данных.
    </p>

    <p>
        Если вы попытаетесь ввести текст или установить флажок в текущей версии примера,
        то увидите, что React проигнорирует ваш ввод. Этого следовало ожидать, потому что мы специально сделали так,
    ...
    <p>
        А теперь давайте зададим окончательный вариант поведения. Нам нужно, чтобы всякий раз,
        когда пользователь изменяет форму, мы обновляли состояние, чтобы отображать
        ...
        каждый раз, когда состояние должно быть обновлено. Чтобы получать уведомление о необходимости обновить состояние, мы можем
        использовать событие <code>onChange</code> на элементах <code>input</code>. Коллбэки, переданные компонентом
        <code>FilterableProductTable</code>, вызовут <code>setState()</code>, и приложение будет обновлено.
    </p>

    <p>
        Это может звучать сложно, но на самом деле нужно добавить всего несколько строк кода.
        В то же время вам всегда понятно, как ваши данные текут во всем приложении.
    </p>
    ...
    <p class="codepen" data-height="600" data-theme-id="0" data-default-tab="js,result" data-user="stzidane"
       data-slug-hash="PvmWpV"
       style="height: 265px; box-sizing: border-box; display: flex; align-items: center; justify-content: center; border: 2px solid; margin: 1em 0; padding: 1em;"
       data-pen-title="Thinking In React: Step 5">
        <span>See the Pen <a href="https://codepen.io/stzidane/pen/PvmWpV/">Thinking In React: Step 5</a>
            by Sergey (<a href="https://codepen.io/stzidane">@stzidane</a>) on <a href="https://codepen.io">CodePen</a>.
        </span>
    </p>
    <script async src="https://static.codepen.io/assets/embed/ei.js"></script>
    ...
    <p>
        Мы надеемся, что эта информация даст вам хорошее представление о том, каким образом нужно размышлять, создавая
        компоненты и приложения с помощью React. Если вам дискомфортно от того, что может понадобиться написать немного больше кода,
        чем вы привыкли, помните, что программисты читают код гораздо больше, чем пишут. Плюс ко всему, этот модульный, ясный код читается очень легко.
        Когда вы начнете создавать большие библиотеки компонентов, вы оцените эту ясность и модульность, а с
        повторным использованием кода ваш собственный код начнёт сокращаться. :)
    </p>
</div>
